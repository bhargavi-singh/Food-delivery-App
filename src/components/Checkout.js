...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      console.log("Add")
      const response = await axios.post("http://127.0.0.1:8082/api/v1/user/addresses",{address:newAddress.value},{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const response = await axios.delete(`http://127.0.0.1:8082/api/v1/user/addresses/${addressId}`,{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      const response = await axios.post("http://127.0.0.1:8082/api/v1/cart/checkout",{addressId:addresses.selected},{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
